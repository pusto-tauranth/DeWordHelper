package com.lyz.dewordhelper.progress;
...
public class TouchProgressView extends View {
    private static final String TAG = "TouchProgressView";//TAG是干吗的？
    ...
    private final int PROGRESS_MAX = 100;//初进程+最大最小值
    ...
    public TouchProgressView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);//attrs属性 看博客
    }
     ...
     * @param radius
     */
    public void setPointRadius(final int radius) {//从何处传来的radius
        if (radius <= 0) {
            ...
            post(new Runnable() {
                ...
                public void run() {
                    if (radius * 2 > getWidth()) {
                        throw new IllegalArgumentException("radius*2 必须小于 view.getWidth() == " + getWidth());//getWidth从何而来？
                    }
                    pointRadius = radius;
                }
            });
     ...
     * @param color
     */
    public void setPointColor(@ColorRes int color) {//color从何处传来?
     ...
     * @param height
     */
    public void setLineHeight(int height) {//height从何处传来
        if (height <= 0) {
    ...
    public void setProgress(int progress) {
        if (progress < 0 || progress > 100) {
            throw new IllegalArgumentException("progress 不可以小于0 或尽量一次不大于100");
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getX() < pointRadius) {//x的坐标值小于圆点半径时
            setProgress(PROGRESS_MIN);
            return true;
        } else if (event.getX() > getWidth() - pointRadius) {
            setProgress(PROGRESS_MAX);//————————————getwidth 直线宽度
            return true;
        } else {
